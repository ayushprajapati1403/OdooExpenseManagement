...
router.post('/approval-flows', authenticateToken, async (req, res) => {
    try {
...
router.get('/approval-flows', authenticateToken, async (req, res) => {
    try {
        if (req.user.role !== 'ADMIN') {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
router.put('/approval-flows/:flowId', authenticateToken, async (req, res) => {
    try {
        if (req.user.role !== 'ADMIN') {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
router.delete('/approval-flows/:flowId', authenticateToken, async (req, res) => {
    try {
        if (req.user.role !== 'ADMIN') {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
router.get('/stats', authenticateToken, async (req, res) => {
    try {
        ...
        const [totalUsers, totalExpenses, pendingExpenses, approvedExpenses, rejectedExpenses, totalAmount] = await Promise.all([
            prisma.user.count({
                where: { companyId: currentUser.companyId }
            }),
