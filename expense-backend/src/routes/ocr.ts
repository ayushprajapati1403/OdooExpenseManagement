  ...
  try {
    ...
    if (mockOcrData.items && mockOcrData.items.length > 0) {
      await prisma.expenseLine.createMany({
        data: mockOcrData.items.map((item: any) => ({
...
router.get('/expense/:expenseId/ocr', authenticateToken, async (req: any, res) => {
  try {
...
router.put('/expense/:expenseId/ocr', authenticateToken, async (req: any, res) => {
  try {
    const { expenseId } = req.params;
...
router.post('/validate', authenticateToken, async (req: any, res) => {
  try {
    ...
    if (ocrData.items && ocrData.items.length > 0) {
      const itemsTotal = ocrData.items.reduce((sum: number, item: any) => sum + (item.amount || 0), 0);
