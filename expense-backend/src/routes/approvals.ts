...
// Approve expense
router.post('/:requestId/approve', authenticateToken, async (req: any, res) => {
  try {
    ...
    if (pendingRequests.length === 0) {
      ...
      const nextRequest = allRequests.find(req => 
        req.stepOrder > currentStepOrder && req.status === 'PENDING'
      );
...
// Reject expense
router.post('/:requestId/reject', authenticateToken, async (req: any, res) => {
  try {
    const { requestId } = req.params;
    const { comment } = req.body;
...
router.get('/expense/:expenseId', authenticateToken, async (req: any, res) => {
  try {
...
router.post('/:requestId/override', authenticateToken, async (req: any, res) => {
  try {
...
router.get('/company/all', authenticateToken, async (req: any, res) => {
  try {
    if (req.user.role !== 'ADMIN') {
      return res.status(403).json({ error: 'Admin access required' });
    }
