...
export class OCRService {
     ...
     */
    async simulateOCRProcessing(imageBuffer, filename) {
...
Total                $8.24`,
     ...
     */
    async processMultipleReceipts(imageBuffers, filenames) {
        try {
            const results = await Promise.all(imageBuffers.map((buffer, index) => this.processReceipt(buffer, filenames[index] || `receipt_${index}.jpg`)));
            return results;
        }
        catch (error) {
