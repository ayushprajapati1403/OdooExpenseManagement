...
export class CurrencyService {
  ...
  async getAllExchangeRates(baseCurrency: string = 'USD'): Promise<ExchangeRate[]> {
    try {
      ...
      const exchangeRates: ExchangeRate[] = Object.entries(rates).map(([currency, rate]) => ({
        currency,
        rate: rate as number,
        timestamp: new Date()
      }));
    ...
    try {
      const results = await Promise.all(
        conversions.map(conversion =>
          this.convertCurrency(conversion.amount, conversion.fromCurrency, conversion.toCurrency)
        )
      );
