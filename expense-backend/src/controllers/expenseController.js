...
export class ExpenseController {
    ...
    async createExpense(req, res) {
        try {
            ...
            if (currency !== user.company.currency) {
                try {
                    const conversion = await this.currencyService.convertCurrency(parseFloat(amount), currency, user.company.currency);
            ...
            try {
                const approvalRequests = await this.approvalService.createApprovalRequests(expense.id, user.companyId);
    ...
    async getExpenseStatistics(req, res) {
        try {
            ...
            const [totalExpenses, pendingExpenses, approvedExpenses, rejectedExpenses, totalAmount, monthlyExpenses] = await Promise.all([
