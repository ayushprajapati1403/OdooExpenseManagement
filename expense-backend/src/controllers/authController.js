...
export class AuthController {
    async signup(req, res) {
        try {
            ...
            // Generate JWT token
            const token = jwt.sign({ userId: result.user.id, email: result.user.email, role: result.user.role }, config.jwt.secret, { expiresIn: config.jwt.expiresIn });
            res.status(201).json({
    ...
    async login(req, res) {
        try {
            ...
            // Generate JWT token
            const token = jwt.sign({ userId: user.id, email: user.email, role: user.role }, config.jwt.secret, { expiresIn: config.jwt.expiresIn });
            res.json({
