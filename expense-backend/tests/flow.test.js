    ...
    describe('Expense Approval Workflow', () => {
        ...
        it('should reject expense', async () => {
            ...
            const pendingResponse = await request(app)
                .get('/api/flows/pending')
                .set('Authorization', `Bearer ${managerToken}`);
            const rejectionRequestId = pendingResponse.body.approvals.find((approval) => approval.expense.id === expenseResponse.body.expense.id)?.id;
        ...
        it('should allow admin override', async () => {
            ...
            const pendingResponse = await request(app)
                .get('/api/flows/pending')
                .set('Authorization', `Bearer ${adminToken}`);
            const overrideRequestId = pendingResponse.body.approvals.find((approval) => approval.expense.id === expenseResponse.body.expense.id)?.id;
